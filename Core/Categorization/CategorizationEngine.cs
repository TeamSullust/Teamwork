...
namespace KitchenPC.Categorization
{
    ...
    {
        private readonly Dictionary<Guid, IIngredientCommonality> commonIngs;
        ...
        private static void CategorizeMeal(Recipe recipe, CategorizationResult result, Analyzer analyzer)
        {
            ...
            else
            {
                ...
                result.Meal_Breakfast = (analysis.FirstPlace.Equals(Category.Breakfast)
                                         || analysis.SecondPlace.Equals(Category.Breakfast));
                result.Meal_Lunch = (analysis.FirstPlace.Equals(Category.Lunch)
                                     || analysis.SecondPlace.Equals(Category.Lunch));
                result.Meal_Dinner = (analysis.FirstPlace.Equals(Category.Dinner)
                                      || analysis.SecondPlace.Equals(Category.Dinner));
                result.Meal_Dessert = (analysis.FirstPlace.Equals(Category.Dessert)
        ...
        private static void CategorizeNutrition(Recipe recipe, CategorizationResult result)
        {
            ...
            foreach (var usage in recipe.Ingredients)
            {
                ...
                if (
                    !(meta.FatPerUnit.HasValue && meta.SugarPerUnit.HasValue && meta.CaloriesPerUnit.HasValue
                      && meta.SodiumPerUnit.HasValue && meta.CarbsPerUnit.HasValue))
                {
                    noMatch = true;
                }

                if (meta.FatPerUnit.HasValue) totalFat += (meta.FatPerUnit.Value * grams) / 100f; //Total fat per 100g

                if (meta.SugarPerUnit.HasValue) totalSugar += (meta.SugarPerUnit.Value * grams) / 100f; //Total sugar per 100g;

                if (meta.CaloriesPerUnit.HasValue) totalCal += (meta.CaloriesPerUnit.Value * grams) / 100f; //Total Calories per 100g

                if (meta.SodiumPerUnit.HasValue) totalSodium += (meta.SodiumPerUnit.Value * grams) / 100f; //Total sodium per 100g

                if (meta.CarbsPerUnit.HasValue) totalCarbs += (meta.CarbsPerUnit.Value * grams) / 100f; //Total carbs per 100g
            }
            ...
            if (!noMatch)
            {
                ...
                result.Nutrition_LowSugar = totalSugar <= (totalCal * .02);
                    //There is no FDA definition of "Low Sugar" (Can estimate 2g of sugar per 100 Cal or less)
                result.Nutrition_LowCalorie = totalCal <= (totalGrams * 1.2);
                    //Definition of Low Calorie is 120 cal per 100g
                result.Nutrition_LowSodium = totalSodium <= (totalGrams * 1.4);
                    //Definition of Low Sodium is 140mg per 100g
                result.Nutrition_LowCarb = totalCarbs <= (totalCal * .05);
                    //No definition for Low Carb, but we can use 5g per 100 Cal or less
            }
        }
        ...
        private void CategorizeSkill(Recipe recipe, CategorizationResult result)
        {
            ...
            result.Skill_Common = recipe.Ingredients.Length >= 3
                                  && recipe.Ingredients.All(i => commonIngs.ContainsKey(i.Ingredient.Id));
            result.Commonality =
                Convert.ToSingle(
                    result.Skill_Common ? recipe.Ingredients.Average(i => commonIngs[i.Ingredient.Id].Commonality) : 0f);

            //Easy: Has the word "easy" in the title, or (prep <= 15min and ingredients <= 5)
            result.Skill_Easy = (recipe.Title.ToLower().Contains("easy")
