...
namespace KitchenPC.Categorization
{
    public class Analyzer
    {
        ...
        private static AnalyzerResult GetWinner(
            Ranking winsBr,
            Ranking winsLu,
            Ranking winsDi,
            Ranking winsDe,
            out Ranking firstPlace,
            out Ranking secondPlace)
        {
        ...
        private void CalcProbability(float cat1count, float cat1total, float cat2count, float cat2total)
        {
            const float s = 1f;
            const float x = .5f;
