        ...
        class Indexer : IDisposable
        {
            ...
            public void Index(IKPCContext context)
            {
                ...
                foreach (var r in snapshot.recipeMap.Values)
                {
                    ...
                    {
                        var nodes = snapshot.recipeList[(int)tag] as List<RecipeNode>;
                        if (nodes == null)
                            snapshot.recipeList[(int)tag] = nodes = new List<RecipeNode>();
                ...
                foreach (var o in loader.LoadIngredientGraph())
                {
                    ...
                    var node = this.snapshot.recipeMap[rid];

                    // New ingredient, create node for it
                    if (!this.snapshot.ingredientMap.TryGetValue(ingid, out ingNode))
                    {
                        nodes = new List<RecipeNode>[Enum.GetNames(typeof(RecipeTag)).Length];
                    ...
                    if (!node.Hidden) //Don't index Hidden recipes
                    {
                        ...
                        {
                            if (nodes[(int)tag] == null)
                            {
                                nodes[(int)tag] = new List<RecipeNode>();
                            }

                            nodes[(int)tag].Add(node); //Add ingredient link to RecipeNode
                        }
                    }
